...
/*-------------------------------------------------------------------------------------
Projeto final- Laboratório de Algoritmos e Estruturas de Dados 1 - LAEDS
...
Data de início: 03/04/2021
Data limite de envio: 08/04/2021
...
#define QUANTIDADE_DIGITOS_PADRAO 13
...
void imprimeMensagem(Mensagem mensagem)
{
	...
	{
		printf("Resultado: ");
		printf("Pista com faixa de pedestres");
	}

	else if(mensagem == naoEncontrado)
	{
		printf("Resultado: ");
		printf("Pista sem faixa de pedestres");
	}
}
...
int main()
{
	...
	int N;												// N do enunciado: Número total de elementos por linha
	...
	int contadorLinhas;									// Para iterar as L linhas
	int L;												// N do enunciado: Número total de linhas

	int padraoAnalisado[13] =  {1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1};	// Mensagem padrão passada no enunciado
	...
	int linhaAtualPossuiFaixa;						// Para falar se na linha atual possui ou não faixa de pedestre
	...
	//---------------------------------------------------------------------------------
	// Início da leitura de dados do arquivo - Obtém o valor de L e verifica se esse valor é válido:

	fscanf(arquivo, "%s", fraseLida);						// Recebe L: Número total de elementos

	if(fraseLida[0] == EOF)									// Se o caracter L recebido for inválido
	{
		printf("ERRO: O valor de L informado é inválido ou inexistente.\n");
		return 0;											// Finaliza o programa
	}

	L = atoi(fraseLida);									// Transforma a String lida em Inteiro

	if( L <= 0)												// L deve ser obrigatoriamente maior que 0
	{
		printf("ERRO: O valor de L não pode ser menor ou igual a 0.\n");
		return 0;	
	}

	// ***********************************************************************************
	// INÍCIO CÓDIGO DO ZIVIANI
	// ***********************************************************************************
    ...
    // ***********************************************************************************
	// FIM CÓDIGO DO ZIVIANI
	// ***********************************************************************************


	//---------------------------------------------------------------------------------
	// Iterador das L linhas

	for(contadorLinhas = 0; contadorLinhas < L; contadorLinhas ++)
	{
		//---------------------------------------------------------------------------------
		...
		if(fraseLida[0] == EOF)									// Se o caracter N recebido for inválido
		{
		...
		{
			printf("ERRO: O valor de N não pode ser menor que 0.\n");
			return 0;	
		}
		...
		linhaAtualPossuiFaixa = 0;
		posicaoAtualComparada = 0;					// Posição comparada do padrão do enunciado
	 	posicaoInicialComparada = 0;				// Posição inicial de comparação

		for(contadorElementos = 0; contadorElementos < N; contadorElementos ++)
		{												// Itera todos os elementos do vetor
			if(matriz[0][contadorElementos] == padraoAnalisado[posicaoAtualComparada])
			{											// Compara cada posição da segunda linha da matriz com o padrão
				if(posicaoAtualComparada == 0)
				{										// Salva a primeira posição, para caso não seja a mensagem, voltar um pouco na iteração do vetor
					posicaoInicialComparada = contadorElementos;
				}

				posicaoAtualComparada ++;				// Incrementa a variável iteradora

				if(posicaoAtualComparada == QUANTIDADE_DIGITOS_PADRAO)
				{										// Se acabou a quantidade de números consecutivos iguais é igual a quantidade de números da mensagem do enunciado, então o padrão foi encontrado
					linhaAtualPossuiFaixa = 1;								
				}
			}
			...
			{
				if(posicaoAtualComparada != 0)
				{										// Se uma sequência foi interrompida, reinicia a contagem da posicaoInicialComparada
					contadorElementos = posicaoInicialComparada;
					posicaoAtualComparada = 0;
				}
			}
		}
		...
		item.Chave = contadorLinhas;
		item.PossuiFaixaPedestre = linhaAtualPossuiFaixa;
		Insere(item, &lista);
	}

	//---------------------------------------------------------------------------------
	// Verifica em todos os itens da lista se algum possui faixa de pedestre

	for(contadorLinhas = 0; contadorLinhas < L; contadorLinhas ++)
	{
		if(lista.Item[contadorLinhas].PossuiFaixaPedestre == 1)
		{
			mensagem = encontrado;
		}
	}
