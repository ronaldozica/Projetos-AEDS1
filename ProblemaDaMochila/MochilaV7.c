...
int main()
{
	...
	int pesoItensContrario[QUANTIDADE_MAX_ITENS];		// Irá conter o peso de cada um dos itens
	int valorItensContrario[QUANTIDADE_MAX_ITENS];		// Irá conter o preço de cada um dos itens
	...
	//---------------------------------------------------------------------------------
	// Recebimento de dados do arquivo - Inversão dos vetores para encontrar mais possibilidades:

	for(contadorItens = 0; contadorItens <= N; contadorItens ++)
	{
		pesoItensContrario[N - contadorItens] = pesoItens[contadorItens - 1];
		valorItensContrario[N - contadorItens] = valorItens[contadorItens - 1];
	}
	...
	//---------------------------------------------------------------------------------
	// Processamento dos dados - análise da solução obtida:
	// Repete o mesmo processo com os vetores ao contŕario para buscar outra possível solução

	for(contadorItens = 0; contadorItens < N; contadorItens ++)
	{															// Percorrer todos os itens -> 0,1,2,3,4,...
		for(contadorSolucao = contadorItens + 1; contadorSolucao < N; contadorSolucao ++)
		{														// Percorrer após o primeiro -> 0,1,2 [...] 0,2,3, [...]
			maiorValor = valorItensContrario[contadorItens];			// Inicializa com maior incial
			pesoMaiorValor = pesoItensContrario[contadorItens];			// Inicializa com peso inicial

			for(contadorSolucaoAux = contadorSolucao; contadorSolucaoAux < N; contadorSolucaoAux ++)
			{													// Se é possível usar esse item sem exceder o peso, use
				if(pesoMaiorValor + pesoItensContrario[contadorSolucaoAux] < W)
				{												// Adiciona o peso, valor desse item
					//printf("%d <- %d\n", maiorValor, valorItensContrario[contadorSolucaoAux]);

					maiorValor += valorItensContrario[contadorSolucaoAux];
					pesoMaiorValor += pesoItensContrario[contadorSolucaoAux];
				}
			}
	...
	for(contadorItens = 0; contadorItens < N; contadorItens ++)	// Imprime a solução verificada
	{
		printf("%d\n", valorItens[contadorItens]);
	}

	printf("Peso ao contrário dos itens:\n");												// \n para facilitar leitura

	for(contadorItens = 0; contadorItens < N; contadorItens ++)	// Imprime a solução verificada
	{
		printf("%d\n", pesoItensContrario[contadorItens]);
	}

	printf("Valor ao contrário dos itens:\n");	

	for(contadorItens = 0; contadorItens < N; contadorItens ++)	// Imprime a solução verificada
	{
		printf("%d\n", valorItensContrario[contadorItens]);
	}
