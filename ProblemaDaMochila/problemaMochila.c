...
int main()
{
	...
	int contadorSalvarSolucao;							// Para iterar e salvar a solução ótima
	...
	int solucaoOtima[QUANTIDADE_MAX_ITENS];				// Irá conter a solução ótima
	int solucaoOtimaGlobal[QUANTIDADE_MAX_ITENS];		// Irá conter a solução ótima definitiva		
	...
	for(contadorItens = 0; contadorItens < N; contadorItens ++)
	{
		solucaoOtima[contadorItens] = 0;						// Inicializa o vetor da solução ótima com 0
		solucaoOtimaGlobal[contadorItens] = 0;					// Inicializa o vetor da solução ótima com 0
	}
	...
	{															// Percorrer todos os itens -> 0,1,2,3,4,...
		...
		{														// Percorrer após o primeiro -> 0,1,2 [...] 0,2,3, [...]
			...
			solucaoOtima[contadorItens] = 1;					// Seta o item atual como 1 no vetor
			...
			{													// Se é possível usar esse item sem exceder o peso, use
				...
				{												// Adiciona o peso, valor desse item
					...
					solucaoOtima[contadorSolucaoAux] = 1;		// Adiciona o item na solução
				}

				else
				{
					solucaoOtima[contadorSolucaoAux] = 0;		// Pula esse item
				}
			}
			...
			if(maiorValor > maiorValorGlobal)
			{
				...
				for(contadorSalvarSolucao = 0; contadorSalvarSolucao < N; contadorSalvarSolucao ++)
				{												// Duplica o vetor solução ótima para salvar a melhor encontrada até então
					solucaoOtimaGlobal[contadorSalvarSolucao] = solucaoOtima[contadorSalvarSolucao];
				}
			...
			for(contadorSalvarSolucao = 0; contadorSalvarSolucao < N; contadorSalvarSolucao ++)
			{
				solucaoOtima[contadorSalvarSolucao] = 0;				// Reseta o vetor de solução para nulo
			}
	...
	for(contadorItens = 0; contadorItens < N; contadorItens ++)	// Imprime a solução verificada
	{
		printf("%d\n", solucaoOtimaGlobal[contadorItens]);
	}
