...
//-------------------------------------------------------------------------------------
// Função calcula maior valor:
// Parâmetro:	
// Retorna:

int calculaMaiorValor(int* pesoItens, int* valorItens, int N, int W)
{
	int maiorValorGlobal = 0;									// Para encotnrar o maior valor
	int contadorItens, contadorSolucao, contadorSolucaoAux;		// Para iterar
	int contadorVerificaSolucao;
	int maiorValor, pesoMaiorValor;								// Para armazenar os resultados e ajudar nos cálculos
			...
			{													// Se é possível usar esse item sem exceder o peso, use
				...
				{												// Adiciona o peso, valor desse item
					...
					for(contadorVerificaSolucao = contadorSolucaoAux; contadorVerificaSolucao < N; contadorVerificaSolucao ++)
					{
						...
						{
							if(peso com esse item excede w mesmo sem o item atual)
							printf("deu ruim dms\n");
						}
					}
...
int main()
{
	...
	maiorValorGlobal = calculaMaiorValor(pesoItens, valorItens, N, W);
